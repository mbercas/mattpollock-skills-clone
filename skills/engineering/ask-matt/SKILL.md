...
- **`/analysis`** — answer a **quantitative question** with a Jupyter notebook that *is the report*: computing over data (parameter sweeps, distributions, perf measurements) where the results — plots, tables, numbers — are the deliverable, committed with outputs. Completes the triad with `/research` (answers by reading) and `/prototype` (answers with throwaway code): `/analysis` answers by measuring, and unlike a prototype its notebook is **kept and maintained** as the code it analyzes changes.## Precondition
